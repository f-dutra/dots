...
drawbar(Monitor *m)
{
	...
	int th = bh - barborderpx * 2;
	int mw = m->ww - barborderpx * 2;
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, mw - tw, y, tw + barborderpx, th, 0, stext, 0);
	}
...
setup(void)
{
	...
	lrpad = drw->fonts->h;
	bh = drw->fonts->h + ubh + barborderpx * 2;
	updategeom();
	/* init atoms */
	...
	drw_clr_create(drw, &clrborder, barbordercolor, barborderalpha);
	/* init bars */
	updatebars();
	updatestatus();
