...
read_Xresources(void) {
	...
	if ((xrm = XResourceManagerString(drw->dpy))) {
		...
		if (XrmGetResource(xdb, "dmenu.normbgcolor", "*", &type, &xval) == True)  /* normal background color */
			colors[SchemeNorm][ColBg] = strdup(xval.addr);
		if (XrmGetResource(xdb, "dmenu.normfgcolor", "*", &type, &xval) == True)  /* normal foreground color */
			colors[SchemeNorm][ColFg] = strdup(xval.addr);
		if (XrmGetResource(xdb, "dmenu.selbgcolor", "*", &type, &xval) == True)  /* selected background color */
			colors[SchemeSel][ColBg] = strdup(xval.addr);
		if (XrmGetResource(xdb, "dmenu.selfgcolor", "*", &type, &xval) == True)  /* selected foreground color */
